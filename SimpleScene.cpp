...
void onMouseDrag( int x, int y ) {
    ...
    /*********************************************************************************/
	movement = (int)sqrt((x - oldX - tempX)*(x - oldX - tempX) + (y - oldY - tempY)*(y - oldY - tempY));
		if (transMode == 'm') {
			if (x - oldX - tempX < 0) {
				movement = -movement;
			}
			...
			glPushMatrix();												// Push the current matrix on GL to stack. The matrix is wld2cam[cameraIndex].matrix().
			glLoadIdentity();
			glMultMatrixd(cow2wld.matrix());							// Multiply the matrix to draw i-th camera.
			if (rotateOn) {
				glRotated(movement, rotx, roty, rotz);
			}
			if (movex && rotateOn==false) {
				glTranslated(movement / 10, 0, 0);

			}
			if (movey&& rotateOn == false) {
				glTranslated(0, movement / 10, 0);

			}
			if (movez&& rotateOn == false) {
				glTranslated(0, 0, movement / 10);
		...
		if (transMode == 'v') {
			...
			printf("moved in drag (%d, %d)\n", x - oldX - tempX, y - oldY - tempY);
			movement = movement / 5;
			if (rotateOn) {

				glPushMatrix();												// Push the current matrix on GL to stack. The matrix is wld2cam[cameraIndex].matrix().
				glLoadIdentity();
				glTranslated(cow2wld.matrix()[12], cow2wld.matrix()[13], cow2wld.matrix()[14]);
				if (cameraIndex == 0) {
					glRotated(movement, 0.5f, 0.0f, -0.5f);
				}
				if (cameraIndex == 1) {
					glRotated(movement, -0.5f, 0.0f, -0.5f);
				}
				...
				if (cameraIndex == 3 || cameraIndex==4) {
					glRotated(movement, 0.0f, 0.0f, 1.0f);
				}
				glTranslated(-cow2wld.matrix()[12], -cow2wld.matrix()[13], -cow2wld.matrix()[14]);
				glMultMatrixd(cow2wld.matrix());							// Multiply the matrix to draw i-th camera.
				glGetDoublev(GL_MODELVIEW_MATRIX, cow2wld.matrix());	// Read the modelview matrix about location and direction set above, and store it in cow2wld matrix.
				...
				glPushMatrix();												// Push the current matrix on GL to stack. The matrix is wld2cam[cameraIndex].matrix().
				glLoadIdentity();
				glMultMatrixd(cam2wld[cameraIndex].matrix());							// Multiply the matrix to draw i-th camera.
				if ((movex || movey) && rotateOn == false) {
					glTranslated((x - oldX - tempX) / 10.0f, 0, 0);
					glTranslated(0, (y - oldY - tempY) / 10.0f, 0);
				}
				if (movez&& rotateOn == false) {
					glTranslated(0, 0, movement / 5);
				}
				glMultMatrixd(wld2cam[cameraIndex].matrix());							// Multiply the matrix to draw i-th camera.
				glMultMatrixd(cow2wld.matrix());							// Multiply the matrix to draw i-th camera.
				glGetDoublev(GL_MODELVIEW_MATRIX, cow2wld.matrix());	// Read the modelview matrix about location and direction set above, and store it in cow2wld matrix.
				glPopMatrix();												// Call the matrix on stack. wld2cam[cameraIndex].matrix() in here.
			}
...
void onKeyPress( unsigned char key, int x, int y ) {
    ...
    if ( cameraIndex >= ( int )wld2cam.size() )
        cameraIndex = 0;
	printf("camera set as %d", cameraIndex);
	...
	if (key == 'r') {
		if (rotateOn) {
			...
			rotateOn = true;
			rotx = (rand()%100)/100.0f;
			roty = (rand()%100)/100.0f;
			rotz = (rand()%100)/100.0f;
			printf("rotate on with axis %lf, %lf, %lf \n", rotx, roty, rotz);
		}
	...
	if (key == 'x') {
		movex = true;
		printf("translate along x axis.\n");
		movey = movez = rotateOn = false;
	}

	if (key == 'y') {
		movey = true;
		printf("translate along y axis\n");
		movex = movez = rotateOn = false;
	}


	if (key == 'z') {
		movez = true;
		printf("translate along z axis\n");
		movey = movex = rotateOn = false;
	}
